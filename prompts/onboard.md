---
description: Reverse-engineer an existing codebase into evidence-cited ADRs + a contributor-ready onboarding guide. Tandem's brownfield on-ramp.
argument-hint: "[path-to-repo] (defaults to current directory)"
---

# /onboard

Point Tandem at a codebase that **already exists** and turn it into the decision
records and documentation Tandem normally produces going forward. This is Tandem's
...
1. **Evidence-cited ADRs** in the target repo's `.claude/docs/decisions/` — the
   durable, Tandem-native artifacts the rest of the workflow consumes.
...
`<target>/.claude/docs/`. Use `--out` when the target already has authored Tandem
decisions you don't want to collide with (e.g. running Tandem on itself).
...
Every extracted ADR uses the standard Tandem ADR frontmatter (so `/rollup`,
...
> Generated by `/tandem:onboard` on YYYY-MM-DD from commit `<short-sha>`.
